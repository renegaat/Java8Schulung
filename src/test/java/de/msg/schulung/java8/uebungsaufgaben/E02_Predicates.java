...
public class E02_Predicates {
	...
	public void testExpoliziteMethodenPersons() {
		...
		// TODO ersetze null durch den Aufruf der passenden Methode dieser Klasse
	...
	public void testPredicatePerson() {
		...
		final List<Person> erwachsenePersonen = filterPersons(persons,  person -> isErwachsenePerson(person)
		);

		assertErwachsenePersonenOk(persons, erwachsenePersonen);

		final List<Person> weiblichePersonen = filterPersons(persons, person -> person.getGeschlecht().equals(Gender.FEMALE));
	...
	public void testMethodenreferenzPerson() {
		...
		// Hinweis: Nutze in den folgenden Übungen die in der Klasse vorgegebenen Hilfsmethoden isErwachsenePerson
		// und isFemalePerson.
	...
	public void testPredicateApple() {
		...
		final List<Apple> greenApples = filterApples(apples, apple -> isGreenApple(apple));

		assertGreenApplesOk(apples, greenApples);

		final List<Apple> heavyApples = filterApples(apples, apple -> isHeavyApple(apple));
	...
	public void testMethodenreferenzApple() {
		...
		// Hinweis: Nutze in dieser Aufgabe die in der Klasse vorgegebenen Methoden
		// isGreenApple und isHeavyApple.
	...
	public void testGenericPredicate() {
		...
		final List<Person> erwachsenePersonen = filterList(persons, this::isErwachsenePerson);

		assertErwachsenePersonenOk(persons, erwachsenePersonen);

		final List<Person> weiblichePersonen = filterList(persons, this::isFemalePerson);
